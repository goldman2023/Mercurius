...
@Component({
  selector: 'app-select-shiftline',
  templateUrl: './select-shiftline.component.html',
  styleUrls: ['./select-shiftline.component.scss'],
})
export class SelectShiftlineComponent implements OnInit {
  ...
  getNewState() {
    ...
    var tempNewObj;
    const prevShiftsState = tempObj?.shiftdefmap;
    if (tempObj != null) {
    ...
    this.isSaved = isEqual(prevShiftsState, temp);
    if (tempObj == null) {
      tempNewObj = {
        bidschename: null,
    ...
    for (
      ...
      if (
        ...
        temp.push({
          ...
          bidshiftmapid:
            this.scheduleListForm.value.allScheduleListData[i].bidshiftmapid,
          shiftdefref:
            this.scheduleListForm.value.allScheduleListData[i].scheduleID,
          shiftdefstartdate:
            moment(this.scheduleListForm.value.allScheduleListData[i].startDate).format('YYYY-MM-DD'),
          shiftdefenddate:
            moment(this.scheduleListForm.value.allScheduleListData[i].endDate).format('YYYY-MM-DD'),
        });
      }
    }
    this.isSaved = isEqual(prevShiftsState, temp);
  ...
  async submit() {
    let storedState = this.getNewState();
    this.localData.setItem('newBidSchedule', JSON.stringify(storedState));
    this.showSuccessAlert('saved', () => {
      if (storedState.roundmap.length > 0) {
        this.getAllEmployeeList();
      } else if (!this.isModal) {
        this.shiftsSelected.emit();
      } else {
  ...
  showSuccessAlert(type: string, callback: () => void) {
    Swal.fire({
      title: 'Success!',
      html: `Shiftlines were ${type} successfully!`,
      icon: 'success',
  ...
  update() {
    let tempNewObj = this.getUpdatedState();
    this.localData.setItem('editBidSchedule', JSON.stringify(tempNewObj));
    this.showSuccessAlert('updated', () => {
      if (tempNewObj.roundmap.length > 0) {
        this.getAllEmployeeList();
      } else {
        this.isModal ? this.modalCtrl.dismiss() : this.shiftsUpdated.emit();
      }
      this.isSaved = true;
    });
  }
          ...
          if (!a) {
            ...
            for (var j = 0; j < tempObj.shiftdefmap.length; j++) {
              if (
                ...
                tempArr.push({
                  id: i,
                  startDate: tempObj.shiftdefmap[j].shiftdefstartdate,
                  endDate: tempObj.shiftdefmap[j].shiftdefenddate,
                  scheduleName: this.all_schedule_list[i].scheduleName,
  ...
  allShceduleBasedOnBidScheduleName() {
    ...
    if (b_schedule_name !== '') {
      this.bidScheduleSer
        .getScheduleNameBasedOnBidScheduleName(b_schedule_name)
        .subscribe(
          (res) => {
  ...
  async checkSch() {
    if (this.isModal) {
      ...
      Swal.fire({
        title: 'Do you want to Create New Shiftline Schedule?',
