...
@Component({
  selector: 'app-select-shiftline-by-bid-manager',
  templateUrl: './select-shiftline-by-bid-manager.component.html',
  styleUrls: ['./select-shiftline-by-bid-manager.component.scss'],
})
export class SelectShiftlineByBidManagerComponent implements OnInit {
final_all_emp=[]
  ...
  valC='mouseenter:mouseleave'
    ...
    config.triggers = 'hover';
      this.route.queryParams.subscribe(params=>{
        this.managerIdForCurrentBidSchdule=params.id
    ...
    ngOnInit() {
      ...
      getAllDataBasedOnScheduleName(){
        this.newBidScheduleSer.getBidScheduleBasedOnBidScheduleIDgetAllBidScheduleData(this.bid_schedule_id).subscribe((res)=>{
          ...
          if(this.getEmpSelectedShiftline.length===this.totalShiftlineScheduleLength){
      ...
      selectShiftLine(selectedShiftLine,index){
        ...
        // this.scrollIdBox.scrollIntoView({
        //   behavior: "smooth",
        //   block: "center",
        // })
        // this.scrollId.scrollIntoView({
        //   behavior: "smooth",
        //   block: "center",
        // })
      ...
      if(this.bidSchedule.shiftdefmap.length===this.selectedShiftLines.length){
        this.widnowTranSer.getBidWindowDataBasedOnempId(this.empId).subscribe((res)=>{
          ...
          for(var k=0;k<temp.length;k++){
            if(temp[k].bidschidref==this.bid_schedule_id && (Number(this.round_id)+ + +1)==temp[k].roundseq_id){
              tempObj=temp[k]
          ...
          tempNewObj={
              ...
              "shiftlinebidstatus": "Manager Completed",
