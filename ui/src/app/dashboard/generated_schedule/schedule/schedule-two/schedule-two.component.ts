...
if(this.focusShiftLine!=null || this.focusShiftLine!=undefined){
  if(this.focusShiftLine.schedule_id==1){
  ...
  if(String(rdo)=='X' || String(rdo)=='x'){
    ...
    for(var i=0;i<this.allShiftData.length;i++){
      if(String(this.allShiftData[i].shiftName)==String(rdo) && Number(shiftlength)==Number(this.allShiftData[i].shift_duration)){
        if(Number(String(this.allShiftData[i].startTime).slice(2))<1){
          if(Number(String(this.allShiftData[i].startTime).slice(0,2))<1){

            return '24'
          }else{
            return Number(String(this.allShiftData[i].startTime).slice(0,2))
          }

        }else{
          if(Number(String(this.allShiftData[i].startTime).slice(0,2))<1){
            return '24'+String(this.allShiftData[i].startTime).slice(2)
          }else{
