...
@Service
public class BackendServiceForBidManagerServiceImpl implements BackendServiceForBidManagerService {
	...
	@Override
	public List<Object> UpdateShiftlineStatus(String emailid) throws ParseException {
		
		long managerid = 0;
		String mailid = emailid;//"ritu.arora@mercuriusinc.com";//emailid
		System.out.println("the email  id is:"+mailid);
		String status = "Incomplete";
		
		 Optional<UserToLogin> managerrecord= login.getdetailForEmail(mailid);
		 UserToLogin record = managerrecord.get();
		  managerid = record.getId();
		 //System.out.println("the manager is:"+managerid);
		 
		 List<BidWindowDuration> temp1 = new ArrayList<BidWindowDuration>();
		 List<Object> bidschedules = schedule.getBasedOnUserId(managerid);
		 int size = bidschedules.size();
		 for(int i=0;i<size;i++)
		 {
			 BidScheduleParamParent bp = (BidScheduleParamParent) bidschedules.get(i);
			...
			// {//comment
			 ...
			 if(managerid!=0)
			 {
				    ...
				    temp1.addAll(window.getEmplyeedetailsBasedonShiftlineStatus(bidid, "Eligible"));
				    System.out.println("temp1 size:"+temp1.size()+" of bidscheduleid:"+bidid);
				    //int s = temp1.size();
