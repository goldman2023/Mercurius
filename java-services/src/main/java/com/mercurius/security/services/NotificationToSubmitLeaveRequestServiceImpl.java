...
import com.mercurius.models.DifferenceBetweenTwoTime;
...
@Service
public class NotificationToSubmitLeaveRequestServiceImpl implements NotificationToSubmitLeaveRequestService {
	...
	@Autowired
	private OtherLeaveRequestsService otherService; 
	
	@Autowired
	private EmployeeBasicWatchScheduleService bwsService;
	...
	public LeaveRequest sendEmailToSumbitLeaveRequest(Long requestId)
		...
		Date reqSubmittedDate = rData.getRequestSubmittedDate();
		String submitDate = dateconvert.Conversion(reqSubmittedDate, 5);
		...
		String subject = requestType+" "+" Request has been submitted on"+" "+submitDate+" "+" . Status is "+status;
		...
		+ "                                        <span style='color:#000000; padding: 10px 20px; margin:0px; display: block;'>Your "+requestType+" request has been submitted on <b>"+submitDate+"</b>. The Request is in <b>"+status+"</b> status.</span>"
		+ "                                      </div>"
		+ "                                      <table align='center' border='0' cellpadding='0' cellspacing='0' class='heading_block' role='presentation' style='mso-table-lspace: 0pt; mso-table-rspace: 0pt;' width='100%'>\r\n"
		...
		+ "                                                <tr>\r\n"
		+ "                                                <tr>\r\n"
		+ "                                                  <td style='width:100%;text-indent: 20px; display: flex ;'><span style='width: 133px; display: block'>Leave End Time: </span> <span style='width: 133px; display: block'>"+leaveEndTime+"</span></td>\r\n"
		+ "                                                </tr>\r\n"
		+ "                                                <tr>\r\n"
		...
		if(rData.getStatus().equals("Approve")||rData.getStatus().equals("Approved"))
		{
			...
			OtherLeaveRequests reqData = new OtherLeaveRequests();
			reqData.setBidschIdRef(requestData.get().getBidschIdRef());
			reqData.setManagerIdRef(requestData.get().getBidManagerId());
			reqData.setEmpIdRef(requestData.get().getEmpId());
			reqData.setEmpInitials(empInitials);
			reqData.setLeaveStartDate(requestData.get().getLeaveStartDate());
			reqData.setLeaveEndDate(requestData.get().getLeaveEndDate());
			//DifferenceBetweenTwoTime diff = new DifferenceBetweenTwoTime();
			//Date totalHours = diff.TimeDifference(lstarttime, lendtime);
			//reqData.setLeaveHours(totalHours);
			reqData.setLeaveType(requestData.get().getRequestType());
			otherService.addOneData(reqData);
