...
public class ShiftCategoryServiceImpl implements ShiftCategoryService{
	...
	@Autowired
	private ShiftCategoryMasterDao shiftmasterdao;

	@Override
	public List<ShiftCategoryMaster> getAll() throws ParseException {
		
		
		List<ShiftCategoryMaster> fct = shiftmasterdao.findAll();
		int n = fct.size();
		
		for(int i=0;i<n;i++)
		{
			ShiftCategoryMaster edata = fct.get(i);
	       ...
	       // edata.setUpdated_date(cdate);
			...
			Long ftype_id = edata.getShcategory_id();
	        
			Optional<ShiftCategoryMaster> ft = shiftmasterdao.findById(ftype_id);
			...
			shiftmasterdao.save(edata);		
			
		}
		List<ShiftCategoryMaster> fdata = shiftmasterdao.findAll();
		return fdata;
	...
	{
		Optional<ShiftCategoryMaster> shCategoryData = shiftmasterdao.findById(sId);
		return shCategoryData;
	}
