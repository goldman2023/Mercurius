...
@Service
public class CronJobForBasicWatchScheduleServiceImpl implements CronJobForBasicWatchScheduleService{
	...
	@Override
	public List<Object> postDataInBasicWatchScheduleTable()
			throws ParseException, UnsupportedEncodingException, MessagingException, UnknownHostException 
	{
		
		List<Long> bidids = parentDao.getBidids("Completed");
		...
		//update
		for(int s=0;s<bidids.size();s++)
		{	
					long Bid = bidids.get(s);
